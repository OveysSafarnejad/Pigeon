...
open class XMPPConversation: NSObject, NSFetchedResultsControllerDelegate {
    ...
    // MARK: Class function
    class var sharedInstance : XMPPConversation {
        struct XMPPConversationSingleton {
            static let instance = XMPPConversation()
        }
        return XMPPConversationSingleton.instance
    }
        ...
        if 0 == sharedInstance.chatList.count {
            if let chatList: NSMutableArray = sharedInstance.getActiveUsersFromCoreDataStorage() as? NSMutableArray {//NSUserDefaults.standardUserDefaults().objectForKey("openChatList")
                chatList.enumerateObjects({ (jidStr, index, finished) -> Void in
                    XMPPConversation.sharedInstance.getUserFromXMPPCoreDataObject(jidStr: jidStr as! String)
                    
                    if let user = XMPPRoster.userFromRosterForJID(jid: jidStr as! String) {
                        XMPPConversation.sharedInstance.chatList.add(user)
                    }
                })
            }
        }
    ...
    fileprivate func getThreadedActiveUsersFromCoreDataStorage() -> NSMutableDictionary? {
        let moc = XMPPMessage.sharedInstance.xmppMessageStorage?.mainThreadManagedObjectContext
                ...
                for message in results! {
                    ...
                    if let chats = archivedMessage.object(forKey: thread) {
                        ...
                        if !(chats as AnyObject).contains(sender) {
                            ...
                            //                                if let user = XMPPRoster.userFromRosterForJID(jid: jidStr as! String) {
                            //                                    XMPPChats.sharedInstance.chatList.addObject(user)
                            //                                }
    ...
    fileprivate func getUserFromXMPPCoreDataObject(jidStr: String) {
        ...
        if XMPPChat.sharedInstance.xmppStream == nil {
    ...
    open class func knownUserForJid(jidStr: String) -> Bool {
        if sharedInstance.chatList.contains(XMPPRoster.userFromRosterForJID(jid: jidStr)!) {
            return true
        } else {
            return false
        }
    ...
    open class func addUserToChatList(jidStr: String) {
        if !knownUserForJid(jidStr: jidStr) {
            sharedInstance.chatList.add(XMPPRoster.userFromRosterForJID(jid: jidStr)!)
            sharedInstance.chatListBare.add(jidStr)
        }
    }
    
    open class func removeUserAtIndexPath(_ indexPath: IndexPath) {
        let user = XMPPConversation.getChatsList().object(at: indexPath.row) as! XMPPUserCoreDataStorageObject
