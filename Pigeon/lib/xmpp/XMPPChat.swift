...
public typealias XMPPChatAuthCompletionHandler = (_ stream: XMPPStream, _ error: DDXMLElement?) -> Void
public typealias XMPPChatConnectCompletionHandler = (_ stream: XMPPStream, _ error: DDXMLElement?) -> Void

public protocol XMPPChatDelegate {
...
open class XMPPChat: NSObject {
    
    var delegate: XMPPChatDelegate?
    var window: UIWindow?
    ...
    var chats: XMPPConversation?
    let presenceTest = XMPPPresence()
    let messageTest = XMPPMessage()
    let rosterTest = XMPPRoster()
    let lastActivityTest = XMPPLastActivity()
    
    var customCertEvaluation: Bool?
    var isXmppConnected: Bool?
    var password: String?
    
    var streamDidAuthenticateCompletionBlock: XMPPChatAuthCompletionHandler?
    var streamDidConnectCompletionBlock: XMPPChatConnectCompletionHandler?
    
    // MARK: Singleton
    
    open class var sharedInstance : XMPPChat {
        struct XMPPChatSingleton {
            static let instance = XMPPChat()
        }
        return XMPPChatSingleton.instance
    }
    ...
    open class func start(_ archiving: Bool? = false, delegate: XMPPChatDelegate? = nil, completionHandler completion:@escaping XMPPChatAuthCompletionHandler) {
        
        
        sharedInstance.setupStream()
        if archiving! {
            XMPPMessage.sharedInstance.setupArchiving()
        }
        if let delegate: XMPPChatDelegate = delegate {
            sharedInstance.delegate = delegate
        }
        XMPPRoster.sharedInstance.fetchedResultsController()?.delegate = XMPPRoster.sharedInstance
    ...
    open func setupStream() {
        ...
        // The XMPPCapabili!ties protocol automatically handles figuring out what these hashes mean,
    ...
    fileprivate func teardownStream() {
        ...
        XMPPMessage.sharedInstance.xmppMessageArchiving!.deactivate()
        xmppStream!.disconnect()
        
        XMPPMessage.sharedInstance.xmppMessageStorage = nil;
        xmppStream = nil;
        xmppReconnect = nil;
        xmppRoster = nil;
    ...
    open func connect(username: String, password: String, completionHandler completion:@escaping XMPPChatConnectCompletionHandler) {
        ...
        if isConnected() {
            print("is connect in one chat")
            streamDidConnectCompletionBlock = completion
            self.streamDidConnectCompletionBlock!(self.xmppStream!, nil)
            return
        }
        
        if ((username.count == 0 || password.count == 0 ) && UserDefaults.standard.string(forKey: kXMPP.myJID) == nil) {
...
extension XMPPChat: XMPPStreamDelegate {
